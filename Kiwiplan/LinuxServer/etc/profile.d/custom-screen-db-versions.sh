...
# Safe for rsync, scp, ansible, forced commands, etc.
# 
# WARNING:
# 
# Changelog:
...
# === SAFETY GUARDS (must be first) ===
[[ $- != *i* ]] && return
[[ -t 0 && -t 1 ]] || return
[[ -n "${SSH_ORIGINAL_COMMAND:-}" ]] && return

# --- Real interactive login from here ---
...
get_postgres_port_map() {
    declare -A port_map
    # Example line: tcp  0  0 0.0.0.0:5432  0.0.0.0:*  LISTEN  1234/postgres
    mapfile -t lines < <(netstat -ltnp 2>/dev/null | grep 'postgres')
    for line in "${lines[@]}"; do
        # Extract port (handles both IPv4 and IPv6)
        port=$(echo "$line" | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\1/')
        # Extract PID
        pid=$(echo "$line" | awk '{print $7}' | cut -d/ -f1)
        if [[ -n "$pid" && -n "$port" && "$pid" =~ ^[0-9]+$ ]]; then
...
get_mysql_versions() {
    if command -v mariadb >/dev/null 2>&1; then
        ver=$(mariadb --version 2>/dev/null | grep -oP 'from \K[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
        if netstat -ltn 2>/dev/null | grep -q ':3306 '; then
            echo -e "${GREEN}MariaDB: $ver → listening on port 3306${NC}"
        else
            echo -e "${GREEN}MariaDB: $ver (not listening on 3306)${NC}"
        fi
    elif command -v mysql >/dev/null 2>&1; then
        ver=$(mysql --version 2>/dev/null | grep -oP 'from \K[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
        if netstat -ltn 2>/dev/null | grep -q ':3306 '; then
...
get_mongodb_version() {
    if command -v mongod >/dev/null 2>&1; then
        ...
        if netstat -ltn 2>/dev/null | grep -q ':27017 '; then
...
get_redis_version() {
    if command -v redis-server >/dev/null 2>&1; then
        ...
        if netstat -ltn 2>/dev/null | grep -q ':6379 '; then
