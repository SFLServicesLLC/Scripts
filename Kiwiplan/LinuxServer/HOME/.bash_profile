...
if [ -f $KIWIBASE/backups/*.sql.gz ];then
   ...
   do 
      ...
      mysql -e "grant all on $NEW_SITE.* to 'kiwisql'@'localhost' identified by 'XXXXX'"
     break
   done
else
...
echo "PASSWORD=XXXXX" >> $KIWIBASE/site_"$NEW_SITE"/data_"$NEW_SITE"/kwsql 
#   echo "PASSWORDX=XXXXX" >> /KIWI/site_$NEW_SITE/data_$NEW_SITE/kwsql   
...
gen_kidds() {
  ...
  echo "KWSQL_PASS=XXXXX" >> $KIWIBASE/site_"$NEW_SITE"/KIDSENV
  echo "" >> $KIWIBASE/site_"$NEW_SITE"/KIDSENV
